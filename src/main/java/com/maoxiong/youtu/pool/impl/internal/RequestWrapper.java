...
public class RequestWrapper implements Comparable<RequestWrapper> {
	...
	private void init(Client requestClient, CallBack<? extends BaseResult> callback) {
		...
		this.request = Optional.ofNullable(requestClient.getRequest()).orElseThrow(
				() -> new RuntimeException("have not set request for client:" + requestClient.getClass().getName()));
	...
	public boolean equals(Object another) {
		...
		return StringUtils.equalsIgnoreCase(requestUrl, anotherUrl)
				&& StringUtils.equalsIgnoreCase(requestParam, anotherParam);
	}
